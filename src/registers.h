...
#define TLATE "2LATE" // servidor rejeita pedido por Quarto de banho já ter encerrado (2LATE)	
...
typedef struct Register {
    ...
    long tid; // identificador no sistema do thread cliente
                      //(cliente, no caso do pedido; servidor, no caso da resposta)
    int dur; // duração, em milissegundos, de utilização (de um lugar) do Quarto de Banho
    ...
    int pl; // no de lugar que eventualmente lhe será atribuído no Quarto de Banho
